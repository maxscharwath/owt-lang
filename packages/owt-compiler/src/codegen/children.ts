...
export function generateComputedExprTextNode(child: Expr, ref: string, ctxVar: string): string {
  ...
  const computeVals = Object.keys(currentValInits)
    ...
    .join(' ');
  let code = `const ${tn} = document.createTextNode(String((() => { const props = ${ctxVar}.props; ${bindVars} ${computeVals} return ${genExpr(child)}; })()));\n`;
  code += `${ref}.appendChild(${tn});\n`;
  const updater = uid('u');
  code += `const ${updater} = () => { const props = ${ctxVar}.props; ${bindVars} ${computeVals} ${tn}.data = String(${genExpr(child)}); };\n`;
  for (const vn of currentVarNames) {
    code += `${ctxVar}.__subs[${JSON.stringify(vn)}] ||= []; ${ctxVar}.__subs[${JSON.stringify(vn)}].push(${updater});\n`;
  }
  return code;
}
